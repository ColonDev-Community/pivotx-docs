...
function useSpaceShooter(onExit: () => void) {
  ...
  const addExplosion = useCallback((x: number, y: number) => {
    explosions.current.push({ x, y, time: 0, duration: 0.8 });
  }, []);

  const restartGame = useCallback(() => {
    gameState.current = {
      playing: true, gameOver: false, score: 0, wave: 1, timeToNextWave: 3
    };
    player.current = {
      x: screenSize.width / 2, y: screenSize.height - 80,
      width: 24, height: 32, health: 100, maxHealth: 100, fireRate: 0.15, lastShot: 0
    };
    bullets.current = [];
  ...
  useGameLoop((dt: number) => {
    ...
    const speed = 400;
    if (keys.current['a'] || keys.current['arrowleft']) p.x = Math.max(0, p.x - speed * dt);
    if (keys.current['d'] || keys.current['arrowright']) p.x = Math.min(W - p.width, p.x + speed * dt);
    if (keys.current['w'] || keys.current['arrowup']) p.y = Math.max(0, p.y - speed * dt);
    if (keys.current['s'] || keys.current['arrowdown']) p.y = Math.min(H - p.height, p.y + speed * dt);

    if (keys.current[' '] && Date.now() - p.lastShot > p.fireRate * 1000) {
      bullets.current.push({
        x: p.x + p.width / 2 - 2, y: p.y, width: 4, height: 12,
        vx: 0, vy: -600, damage: 25, color: '#00ff00'
      });
      p.lastShot = Date.now();
    }
    ...
    enemies.current = enemies.current.filter(enemy => {
      ...
      if (Date.now() - enemy.lastShot > enemy.fireRate * 1000 && Math.random() < 0.3 * dt) {
        enemyBullets.current.push({
          x: enemy.x + enemy.width / 2 - 1.5, y: enemy.y + enemy.height,
          width: 3, height: 8, vx: 0, vy: 300, damage: 15, color: '#ff4444'
        });
    ...
    powerUps.current.forEach((powerUp, powerUpIndex) => {
      if (checkCollision(powerUp, p)) {
        ...
        switch (powerUp.type) {
          ...
          case 'fireRate': p.fireRate = Math.max(0.08, p.fireRate - 0.02); break;
          case 'damage': break;
        }
  ...
  return (
    <div style={{ 
      ...
      <PivotCanvas width={W} height={H} background="#0a0a0a">
        <PivotRectangle position={{ x: 0, y: 0 }} width={W} height={H} fill="#0a0a0a" />
